...
import { stablesClient, getExchangeRate, createTokenTransferTransaction, SUPPORTED_STABLECOINS } from "@/lib/stables/client";
import OffRampHistory from "@/components/perena/OffRampHistory";
import { useWallet } from "@solana/wallet-adapter-react";
import { Transaction } from "@solana/web3.js";
import config from "@/lib/stables/config";

export default function StablesWallet({
  walletAddress,
}: {
  ...
  const [selectedToken, setSelectedToken] = useState<string>(config.defaultToken);

  // Environment variables
  const FRAMP_POOL_WALLET = config.poolWalletAddress; // From config
  
  // Fee percentages from config
  const onRampFeePercentage = config.onRampFeePercentage;
  const offRampFeePercentage = config.offRampFeePercentage;
  ...
  // Load available banks from config
  const banks = config.banksByCountry['Nigeria'] || [];
  ...
  useEffect(() => {
    if (activeTab === "onramp") {
      getExchangeRate(selectedToken, "USD").then((rate) => setExchangeRate(rate));
    } else if (activeTab === "offramp") {
      getExchangeRate(selectedToken, currency).then((rate) => setExchangeRate(rate));
    }
  }, [activeTab, currency, selectedToken]);
  ...
  async function loadWalletData() {
    ...
    try {
      ...
      for (const tokenSymbol of Object.keys(SUPPORTED_STABLECOINS)) {
        console.log(`Fetching ${tokenSymbol} balance for wallet: ${walletAddress}`);
        try {
          const balance = await stablesClient.getBalance(walletAddress, tokenSymbol);
          console.log(`${tokenSymbol} balance result:`, balance);
          newBalances[tokenSymbol] = balance;
        } catch (tokenError) {
          console.error(`Failed to load ${tokenSymbol} balance:`, tokenError);
          newBalances[tokenSymbol] = null;
        }
      }
      ...
      const poolInfoResult = await stablesClient.getPoolInfo();
      ...
      showToast({
        ...
        description:
          "Could not retrieve your token balances. Please try again later.",
      });
    } finally {
      setIsLoading(false);
  ...
  async function handleOnRamp(e: React.FormEvent) {
    ...
    try {
      ...
      showToast({
        ...
        description: `${result.amount.toFixed(2)} ${selectedToken} has been added to your wallet.`,
  ...
  async function handleOffRamp(e: React.FormEvent) {
    ...
    if (parseFloat(offRampAmount) > selectedBalance) {
      showToast({
        variant: "destructive",
        title: "Insufficient balance",
        description: `You don't have enough ${selectedToken} for this withdrawal.`,
      });
      return;
    }
    ...
    try {
      ...
      console.log(`Creating transfer transaction from ${walletAddress} to ${FRAMP_POOL_WALLET} for ${transferAmount} ${selectedToken}`);
      ...
      <div className="bg-white dark:bg-background/50 backdrop-blur-md border border-black/10 dark:border-white/10 shadow-sm rounded-xl overflow-hidden">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-black/60 dark:text-white/60 text-sm">
                Select a stablecoin to on-ramp or off-ramp
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5">
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                {Object.entries(SUPPORTED_STABLECOINS).map(([symbol, token]) => (
            ...
            <div className="text-3xl font-bold text-black dark:text-white">
              {isLoading ? (
                ...
                `${(balances[selectedToken] || 0).toFixed(6)} ${selectedToken}`
              )}
            </div>
                    ...
                    <div>
                      ...
                      {onRampAmount && (
                        <p className="text-xs text-black/60 dark:text-white/60 mt-1">
                          ...
                          {selectedToken} (after {onRampFeePercentage}% fee)
                        </p>
                      )}
                    </div>
                  ...
                  <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs mt-1">
                          {process.env.NODE_ENV === 'development' 
                            ...
                            : `You will need to sign a transaction with your connected wallet to authorize the transfer of ${selectedToken} to Framp's pool wallet.`}
                        </p>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full rounded-md border border-black/20 dark:border-white/20 bg-white dark:bg-black/30 px-3 py-2 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#7b77b9]"
                      >
                        {config.supportedFiatCurrencies.map(fiat => (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-black/60 dark:text-white/60 mt-1">
                        Make sure to select the bank associated with your account
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        id="bank-code"
                        type="text"
                        placeholder="Bank code (automatically filled)"
                        value={bankCode}
                        className="w-full rounded-md border border-black/20 dark:border-white/20 bg-white dark:bg-black/30 px-3 py-2 text-black dark:text-white placeholder-black/50 dark:placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-[#7b77b9]"
                        readOnly
                        required
        ...
        <div className="flex flex-col text-sm text-black/60 dark:text-white/60 border-t border-black/10 dark:border-white/10 p-5">
          <div className="flex items-start gap-2 mb-2">
            ...
            <p>
              {selectedToken === 'USD_STAR' 
                ? `USD* earns yield from Perena's DeFi pools. Current estimated APR is ${poolInfo?.apr || "~5.2"}%.`
                : `${selectedToken} can be easily on-ramped and off-ramped through Framp.`}
            </p>
          </div>
        </div>
      </div>
